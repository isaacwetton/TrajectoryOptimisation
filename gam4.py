...
closest = (constants.EPOCH, 1000 * constants.R_JUPITER, orbiter)
...
while Jdist <= initial_Jdist:
    ...
    if np.linalg.norm(orbiter.position - moon_obj[3].position) < closest[1]:
        closest = (T, np.linalg.norm(orbiter.position - moon_obj[3].position), orbiter)

# Print closest approach time, distance and orbiter object
print(closest)

# Move the orbiter towards Callisto, to an altitude of 50km, and evolve backwards
orbiter = closest[2]
T = closest[0]
for i in range(4):
    moon_states[i] = moons[i](T)
...
orb_to_cal_unitvec = (moon_obj[3].position - orbiter.position) / np.linalg.norm(moon_obj[3].position - orbiter.position)
orbiter.position += orb_to_cal_unitvec * (closest[1] - constants.R_CALLISTO - 50)
Jdist = np.linalg.norm(orbiter.position - jupiter.position)
...
# Print results
print("Orbiter initial position vector: ", orbiter.position)
print("Orbiter initial velocity vector: ", orbiter.velocity)
print("Orbiter initial velocity: ", np.linalg.norm(orbiter.velocity))
