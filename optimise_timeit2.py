...
simann_decs = np.exp(np.linspace(-12, 0, 100))
simann_times = []
simann_errors = []
...
monte_sols = np.rint(np.exp(np.linspace(2, 15, 100)))
monte_times = []
monte_errors = []

for i in range(len(monte_sols)):
    t_monte = timeit("result, vals = optimise.montecarlo(quadratic, 0, " + str(int(monte_sols[i])) + ", 10, (-5.0, 5.0))",
                     setup=setup, number=tests)
