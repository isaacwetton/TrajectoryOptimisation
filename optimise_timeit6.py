...
interp = RegularGridInterpolator((deltas, phis), both2, bounds_error=False, fill_value=None)
...
simann_decs = [0.1, 0.04, 0.02, 0.015, 0.0075, 0.004]
simann_times = []
simann_errors = []

for i in range(len(simann_decs)):
    t_simann = timeit("result, vals = optimise.simann(find_semimajor, 0.1, " + str(simann_decs[i]) + ", 0, 1.0, (0, 2*np.pi), (-0.10, 0.10), (59092.3, 59094.3))",
                      setup=setup, number=tests)
    simann_times.append(t_simann / tests)
    errors = 0
    for j in range(tests):
        result, vals = optimise.simann(find_semimajor, 0.1, simann_decs[i], 0, 1.0, (0, 2*np.pi), (-0.10, 0.10), (59092.3, 59094.3))
        print(vals)
        print(result)
        errors += abs(result)
...
# Save simann data
f = open("data/timeit6simann.dat", "wb")
