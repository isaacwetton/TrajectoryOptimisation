...
monte_sols = [10, 25, 50, 75, 100, 150]
monte_times = []
monte_errors = []

for i in range(len(monte_sols)):
    t_monte = timeit(
        "result, vals = optimise.montecarlo(find_semimajor, 0, " + str(monte_sols[i]) + ", 1, (0, 2*np.pi), (-0.10, 0.10), (59092.3, 59094.3))",
        setup=setup, number=tests)
    monte_times.append(t_monte / tests)
    errors = 0
    for j in range(tests):
        result, vals = optimise.montecarlo(find_semimajor, 0, int(monte_sols[i]), 1, (0, 2*np.pi), (-0.10, 0.10), (59092.3, 59094.3))
        print(vals)
        print(result)
        errors += abs(result)
...
# Save monte data
f = open("data/timeit5monte.dat", "wb")
