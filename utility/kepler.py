...
class KeplersSolverDidNotConverge(Exception):
    ...
    def __init__(self, num_iter: int, delta_ecc_anomaly: float, relative_change: float, mean_anomaly: float,
                 eccentricity: float, message: str = None):
        """Constructor.
        ...
        """
        super().__init__(
            "Convergence failed at {} iterations with dE={}, relative change={} (called with M={} ecc={}){}".format(
                num_iter, delta_ecc_anomaly, relative_change, mean_anomaly, eccentricity,
...
class BasicKeplersEquationSolver:
    ...
    def __call__(self, mean_anomaly: float, eccentricity: float) -> float:
        ...
        while (relative_change > self.relative_change_epsilon) and (iter < self.max_iter):
            ...
            delta_ecc_anomaly = (mean_anomaly + eccentricity * np.sin(ecc_anomaly) - ecc_anomaly) / (
                        1 - eccentricity * np.cos(ecc_anomaly))
...
class KeplerianEllipticalOrbit:
    ...
    def __init__(self, a: float, eccentricity: float, inclination: float, long_asc_node: float, arg_periapsis: float,
                 mean_anomaly0: float, epoch: float, mu: float, kepler_solver=BasicKeplersEquationSolver(1e-6, 100)):
        """Constructor
    ...
    def calculate_state(self, t: float) -> np.ndarray:
        ...
        self.true_anomaly = 2.0 * np.arctan(
            np.sqrt((1 + self.eccentricity) / (1 - self.eccentricity)) * np.tan(self.eccentric_anomaly * 0.5))
        self.flight_path_angle = np.arctan2(self.eccentricity * np.sin(self.true_anomaly),
                                            1.0 + self.eccentricity * np.cos(self.true_anomaly))
        ...
        s = [r * (np.cos(pos_angle) * np.cos(self.long_asc_node) - np.sin(pos_angle) * np.cos(
            self.inclination) * np.sin(self.long_asc_node)),
             r * (np.cos(pos_angle) * np.sin(self.long_asc_node) + np.sin(pos_angle) * np.cos(
                 self.inclination) * np.cos(self.long_asc_node)),
             r * np.sin(pos_angle) * np.sin(self.inclination),
             v * (-np.sin(vel_angle) * np.cos(self.long_asc_node) - np.cos(vel_angle) * np.cos(
                 self.inclination) * np.sin(self.long_asc_node)),
             v * (-np.sin(vel_angle) * np.sin(self.long_asc_node) + np.cos(vel_angle) * np.cos(
                 self.inclination) * np.cos(self.long_asc_node)),
